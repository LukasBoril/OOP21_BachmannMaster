...
public class EntscheidungsKnopf4 extends Application {
    ...
    private Node createMenuPane(Stage stage) {
        ...
        int strategyParam = 0;
        MenuItem menuObjStream = createMenuItem("ObjectStream I/O", event -> setStrategy(stage, strategyParam), KeyCode.DIGIT0, KeyCombination.CONTROL_DOWN);
        MenuItem menuFileStream = createMenuItem("FileStream I/O", event -> setStrategy(stage, strategyParam), KeyCode.DIGIT0, KeyCombination.CONTROL_DOWN);

        menuFile.getItems().addAll(menuOpen, menuSave, menuExit);
        menuStrategy.getItems().addAll(menuObjStream,menuFileStream);
    ...
    private void setStrategy(Stage stage, int strategyParam) {
